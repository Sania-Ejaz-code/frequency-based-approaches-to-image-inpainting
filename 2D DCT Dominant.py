...
# CHANGED: use DCT instead of FFT
from scipy.fft import dctn, idctn  # (replaces: fft2, ifft2, fftshift, ifftshift)
...
# === DCT-based inpainting with dominant frequency preservation (same interface) ===
def DCT_inpaint(image, mask, num_iters=400, k_frac=0.07, eps=1e-12, use_window=True):
    """
    Gerchberg–Papoulis inpainting with dominant-frequency constraint (DCT version).
    image: float64 in [0,1]
    mask:  1 = missing, 0 = known
    num_iters: iterations of projections
    k_frac: fraction of largest-magnitude DCT coefficients to keep (0.02–0.10 works well for periodic textures)
    """
    ...
    # Iterate: (A) go to DCT, keep top-K magnitudes, (B) return to space and enforce known pixels
    for _ in range(num_iters):
        # CHANGED: DCT instead of FFT
        X = dctn(x * W, type=2, norm='ortho')          # windowed DCT to tame boundaries
        mag = np.abs(X).ravel()
        if K < mag.size:
            ...
            X = X * keep
        # CHANGED: inverse DCT
        x_recon = idctn(X, type=2, norm='ortho') / (W + 1e-8)   # undo window (approx)
...
image_path = "Images/P_Zigzagbar Hor.png"  # Replace with  own image path

# A) 30% random pixels missing
#mask_path = "random:0.80"

# B) One 80x120 rectangular hole at random location
#mask_path = "block:50x50"
...
#mask_path = "irregular:strokes=10,t=10-30,seed=42"
...
#verticl bars
image_inpainted = DCT_inpaint(image,    mask, num_iters=300, k_frac=0.000 , use_window=False)
...
axs[1].set_title("With 60% Masked Region")
axs[1].axis('off')
